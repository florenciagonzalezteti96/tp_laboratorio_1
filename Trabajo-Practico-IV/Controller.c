...
int controller_removeEmployee(LinkedList* pArrayListEmployee, LinkedList* pArrayListRemovedEmployees)
{
    ...
    char confirmacion;
    Employee* auxEmpleado;
    Employee* auxEmpleadoEliminado;
    int indiceEmpleado;

    if(pArrayListEmployee != NULL && pArrayListRemovedEmployees != NULL)
    {
        isEmpty = ll_isEmpty(pArrayListEmployee);
        if(isEmpty != 1)
        {
            ...
            if(auxEmpleado != NULL)
            {
                ...
                if(confirmacion == 's')
                {
                    ...
                    auxEmpleadoEliminado = (Employee*)ll_pop(pArrayListEmployee,indiceEmpleado);
                    showOneEmployee(auxEmpleadoEliminado);
                    retorno = ll_add(pArrayListRemovedEmployees,auxEmpleadoEliminado);
                }
                else if(confirmacion == 'n')
                {
                    retorno = 2;
...
int mostrarMenuListar(LinkedList* pArrayListEmployee, LinkedList* pArrayListRemovedEmployee)
{
    int retorno = -1;
    int opcion;
    if(pArrayListEmployee != NULL && pArrayListEmployee != NULL)
    {
        system("cls");
        opcion = getInt("MENU DE LISTAS\nIngrese una opcion:\n1.Mostrar lista de empleados de la empresa principal.\n2.Mostrar lista de empleados dados de baja.\n");
        switch(opcion)
        {
        case 1:
...
void* getEmployeeByHoursWorked(LinkedList* pArrayListEmployee, int hoursWorked)
{
    ...
    if(pArrayListEmployee != NULL)
    {
        ...
        {
            auxEmployee = (Employee* ) ll_get(pArrayListEmployee, i);
            if(hoursWorked == auxEmployee->horasTrabajadas)
            {
                return auxEmployee;
                break;
            }
...
void* getEmployeeByNombre(LinkedList* pArrayListEmployee, char* name)
{
    ...
    if(pArrayListEmployee != NULL)
    {
        ...
        {
            auxEmployee = (Employee* ) ll_get(pArrayListEmployee, i);
            if(strcmp(name,auxEmployee->nombre) == 0)
            {
                return auxEmployee;
                break;
            }
